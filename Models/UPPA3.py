"""
Final UPPA version, referred to as GSCD-Unet in this project.

It combines gradient-guided spatial attention, 3D space-to-depth downsampling,
SE channel reweighting, and Group Normalization for batch-size-one training.
"""
import torch
...
class UPPA3(nn.Module):
    ...
    def double_conv(self, in_channels, out_channels):
        return nn.Sequential(
            ...
            nn.GroupNorm(num_groups=int(out_channels / 8), num_channels=out_channels),
    ...
    def forward(self, x):
        ...
        # Upsampling path
        up5 = self.up5(conv4)
        mid2_down = self.mid2_down(conv2)
        up5 = torch.cat([mid2_down, up5, conv3], dim=1)

        conv5= self.conv5(up5)

        up6 = self.up6(conv5)
        mid1_down = self.mid1_down(conv1)

        up6 = torch.cat([mid1_down, up6, conv2], dim=1)
